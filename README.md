...
给 Minecraft 里的 **Numen AI 同伴**绑定一个 QQ 号:你在 QQ 上私聊它,它在游戏里动手,干完自己回消息汇报。拉进群里也一样用。

```
你的QQ  ──NapCatQQ (OneBot 11 WebSocket)──▶  numen-qq  ──NumenGateway.enqueue──▶  同伴的消息队列
你的QQ  ◀──send_private_msg / send_group_msg──  numen-qq  ◀──send_qq_message 工具──  同伴自己决定回复
```

- **入站**:白名单内的私聊/群消息包装成 `[QQ私聊 · 昵称(QQ号)] 内容` 或 `[QQ群123456 · 昵称] 内容`,进入同伴的消息队列——同伴空闲立刻开工,忙的时候在下一个动作间隙看到;
- **出站**:本模组向 numen-api 注册一个 `send_qq_message` 工具,私聊回私聊、群里回群里(标签里带号码,模型自己认)。同伴回不回、回什么、什么时候回,由它的大脑决定——对它来说,给 QQ 发消息和挥一下镐子是同一种动作;
...
| `user_whitelist` | 接收哪些 **QQ 号的私聊**——填上你自己的号,私聊机器人就能指挥同伴 |
| `companion_name` | 消息投递给哪个同伴,留空 = 名册上的第一个 |

两个白名单同一条铁律:**空列表 = 一条也不收**。每个群、每个私聊对象都是显式授权——放进来的人花的是你的大模型 token。
